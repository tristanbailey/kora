...
class Fishpig_Wordpress_Model_Image extends Fishpig_Wordpress_Model_Post_Attachment_Abstract
{
	...
	{
		$url = Mage::helper('wordpress')->getWpOption('fileupload_url');
		
		if (!$url) {
			foreach(array('upload_url_path', 'upload_path') as $config) {
				if ($value = Mage::helper('wordpress')->getWpOption($config)) {
					//$url = Mage::helper('wordpress')->getBaseUrl($value);
					$url = $value;
					break;
				}
			}
			...
			if (!$url) {
				$url = Mage::helper('wordpress')->getBaseUrl('wp-content/uploads/');
			}
		}
