...
class Fishpig_Wordpress_Helper_Router extends Fishpig_Wordpress_Helper_Abstract
{
	...
	public function isCategoryUri($uri)
	{
		...
		return preg_match('/^' . str_replace('/', '\/', trim(Mage::helper('wordpress/router')->getCategoryBase(), '/')) . '\/' . $this->getPermalinkStringRegex() . '$/i', $uri);
	}
	...
	{
		return preg_match('/^' . $this->getTagBase() . '\/' . $this->getPermalinkStringRegex() . '$/', $uri);
	}
	...
	{
		return Mage::helper('wordpress')->getWpOption('category_base', 'category');
	}
