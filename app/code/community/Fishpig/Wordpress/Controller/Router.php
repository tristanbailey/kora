...
class Fishpig_Wordpress_Controller_Router extends Mage_Core_Controller_Varien_Router_Standard
{
	...
	protected function _match($uri)
	{
		...
		if (!$uri && !Mage::helper('wordpress/post')->getPostId()) {
			...
			return $this->getRequest()->setControllerName('misc')->setActionName('forward');
		}
		else if ((strpos($uri, 'wp-content/') !== false) || (strpos($uri, 'wp-includes/') !== false)) {
			return $this->getRequest()->setControllerName('misc')->setActionName('forwardFile')->setParam('uri', $uri);
		}
	...
	protected function _initStaticRoutes()
	{
		$this->addStaticRoute('author', 'author')
			->addStaticRoute(Mage::helper('wordpress')->getWpOption('tag_base', 'tag'), 'tag')
			...
			->addStaticRoute('wp-pass.php', 'misc', 'applyPostPassword');
